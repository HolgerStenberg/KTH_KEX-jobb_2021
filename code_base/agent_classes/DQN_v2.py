...
class DQN_agent:
	...
	def __init__(self, agent_state_size, agent_action_size, rows, columns):

		self.state_size = agent_state_size   # used for input  nodes in NN
		self.action_size = agent_action_size # used for output nodes in NN
	...
	#needs to be affected by num of cols and rows and number of robots.
		...
		for i in wanted_state:
			...
			state += ((i[0]-1)*self.__COLUMNS + (i[1]-1))*multiplier + matrix_size*i[2]
	...
	def act(self, state):
		...
		if np.random.rand() <= self.exploration_memory[state_as_number]:
			...
			if self.exploration_memory[state_as_number] > self.epsilon_min:
				self.exploration_memory[state_as_number] -= 0.01
